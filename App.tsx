...
const App: React.FC = () => {
  ...
  // Stories Logic - Firestore stories collection থেকে
  const storyMovies = useMemo(() => {
    if (storyItems.length > 0) {
      return storyItems
        .map(s => movies.find(m => m.id === s.movieId))
        .filter(Boolean) as Movie[];
    }
    // Fallback: latest 4 movies
  ...
  const displayedMovies = useMemo(() => {
     ...
     if (activeCategory === 'All') {
         ...
         // Exclusive category ক্লিক করলে:
         // - category === 'Exclusive' OR isExclusive === true দুটোই দেখাবে
         filtered = filtered.filter(m => m.category === 'Exclusive' || m.isExclusive === true);
     } else {
         // অন্য category (Korean Drama, Series) - শুধু সেই category দেখাবে
  ...
  return (
    ...
    >
      ...
      <main className={`px-4 max-w-7xl mx-auto relative z-10 ${activeTab === 'home' ? '-mt-10' : 'pt-20'}`}>
        ...
        {activeTab === 'home' && (
            <>  
                {/* 1. Trending - isTop10 flag দিয়ে ফিল্টার করা */}
                <TrendingRow movies={top10Movies} onClick={handleMovieClick} />

                {/* 2. Story Circles - Firestore stories collection থেকে */}
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {storyMovies.slice(0, 8).map((movie, index) => (
                            <StoryCircle 
                                key={movie.id} 
                                movie={movie} 
                                index={index} 
                ...
                {/* 3. Bengali Notice Bar - ✅ channelLink pass করা */}
                <NoticeBar channelLink={appSettings.channelLink} />
